...
def Compress(filePath):
    ...
    for event, elem in ET.iterparse(filePath, events=("start", "end")):
        if event == "start":
            ...
            if len(elem.attrib) > 0: # ATTRIBUTE VE DEGERLERI
                for key, value in elem.attrib.items(): #attributeler sözlüğe atılıyor
                    ...
                    if (value != "" or value is not None) and (attPath not in contDict):  # container sözlüğe ekleniyor
                        contDict[attPath] = ["C" + str(contId), key]
                        contId += 1
                    struct += contDict[attPath][0] + " "
                    struct += "/ " #ATTRIBUTE bitti.

            if contId > 0: # ELEMENT DEGERLERI
                contElmpath = contElmpath + "/" + elem.tag
            if (elem.text != "" or elem.text is not None) and (
                contElmpath not in contDict):  # container sözlüğe ekleniyor
                contDict[contElmpath] = ["C" + str(contId)]
                contId += 1
            struct += contDict[contElmpath][0] + " "

        if event == "end":
            struct += "/ " # ELEMENT bitti
